...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  */
  if (nrhs < 2)
    mexErrMsgTxt("SPIKES2EXP requires at least 2 input arguments!\n");

  if (nlhs != 1) 
    mexErrMsgTxt("SPIKES2EXP requires 1 output argument!\n");
  ...
  if ( nDIM > 0 && !mxIsStruct(C_IN) )
    mexErrMsgTxt("SPIKES2EXP requires that C is a struct\n");

  if ( nDIM == 0 )
    mexErrMsgTxt("SPIKES2EXP: nothing to do since there are 0 channels!\n");

  if ( nDIM> 0 && min(m,n) != 1 )
    mexErrMsgTxt("SPIKES2EXP: C is not a struct vector.\n");
...
/*        printf("spiketimes found (%i)!\n",i_spiketimes); */
  ...
  */
  if ( getDoubleVector(t_IN,&sample_time,&nSampleTimePoints) < 0 )
    mexErrMsgTxt("SPIKES2EXP requires that 't' is a sortet vector!\n");

  tau1=30e-3;
  if ( nrhs > 2 ) {
    if ( getDouble(tau1_IN,&tau1) < 0 )
      mexErrMsgTxt("SPIKES2EXP requires that 'tau1' is a double scalar!\n");
    if ( tau1 <=0 ) mexErrMsgTxt("SPIKES2EXP requires that tau1 > 0!");
  }
  ...
  */
  ...
  if ( nrhs > 3 ) {
    if ( getDouble(tau2_IN,&tau2) < 0 )
      mexErrMsgTxt("SPIKES2EXP requires that 'tau2' is a double scalar!\n");
    if ( tau2 <=0 ) mexErrMsgTxt("SPIKES2EXP requires that tau2 > 0!");
  }
...
/*              printf("%g ",spike_time[i]); */
        ...
        mexErrMsgTxt("SPIKES2EXP: invalid value for dt!\n");
      }
    }
