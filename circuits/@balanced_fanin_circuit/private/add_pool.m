function [nmc, p] = add_pool(nmc, varargin)
...
neuronType = INH * ones(prod(np.size),1);
rp=randperm(length(neuronType));
neuronType(rp(1:ceil(length(rp)*np.frac_EXC)))=EXC;

nExc = length(find(neuronType==EXC));
nInh = length(find(neuronType==INH));
...
np.isInput = 0;
if ~isempty(strfind(np.type, 'Input')) 
  np.isInput = 1;
end
...
if VERBOSE_LEVEL > 0
  fprintf('Created %ix%ix%i pool %i (frac_EXC=%g, Iinject=%g-%g): %i %s (%i ecx, %i inh)\n', ...
      np.size(1), np.size(2), np.size(3), p, ...
      np.frac_EXC, np.Neuron(EXC).Iinject(1), np.Neuron(EXC).Iinject(2), ...
      length(np.neuronIdx), np.type, nExc, nInh);
end
