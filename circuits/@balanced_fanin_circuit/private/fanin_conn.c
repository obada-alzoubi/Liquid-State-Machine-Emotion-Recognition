...
void makeconn(uint32 **postidx, uint32 **preidx, int *nconn,
  ...
  unsigned i, j, j2;  /* index variables: i ... post, j,j2 ... pre */

  int count, full, h, l, pre, found; /* for binary search */
  ...
  for (i=0; i<ndest; i++) {
    ...
    */
    if (finiteLambda) {
      ...
      if (sumP) {
         ...
         /* if lambda == 0 all connections are equi-probable */
         for (p=P,j=0; j<nsrc; j++) {
           *(p++) = 1.0/nsrc;
         }
      }
    ...
    */
	...
	count = 0;
        ...
        /* if chosen connections is already existing choose again randomly */
        count++;
      }

      /* if 10 times the randomly chosen connections alrady existed
         choose most likely nonexisting connection */
      if (count > 10) {
         pre = -1;
	 pmax = -1;
	 /* loop through all possible connections with source neurons */
         for (p=P,j2=0; j2<nsrc; j2++) {
            /* check if the connection is more likely then the preceeding ones */
            if ((*p) > pmax) {
   	       /* check if the connection is already used or a self loop */
               if ( !usedSrc[j2] && srcidx[j2]!=destidx[i] ) {
                  pmax = (*p);
		  pre = j2;
	       }
	    }
	 ...
	 /* check if a free connection was found or aeverything is full */
	 if (pmax >= 0) {
            (*postidx)[*nconn] = destidx[i];
            (*preidx)[*nconn] = srcidx[pre];
            (*nconn)++;
	    ...
	    /* all connections are already used */
            full = 1;
	 }
