...
%PLOT Plots a pool circuit template.
%   PLOT(CT) plots a matlab figure of the pool circuit template CT in
%   the current figure GCF.
%
%   PLOT(CT1,CT2,...) plots matlab figures of the pool circuit templates
%   CT1, CT2 etc. starting with the current figure GCF. Each
%   template is plotted in its own figure with increasing figure
%   number.
%
...
%
%	      plots for the pool circuit templates CT{1} and CT{3} no
...
for nCT = 1:length(CT)
   ...
   if sub_plot(3)
      for i = 1:length(CT{nCT}.conn)
        for j = 1:length(CT{nCT}.conn(i).parameters.Synapse)
          CT{nCT}.conn(i).parameters.Synapse(j).W =  CT{nCT}.conn(i).parameters.Synapse(j).W * ...
	  					 CT{nCT}.conn(i).parameters.Wscale;
        end
      end
