...
if (nlinear == 2)
   ...
   nO1 = 1;
   for nO2 = 1:orders
      uu(:,nO2) = u(nO1:end-orders+nO1).*u(nO2:end-orders+nO2);
   end

   for nO1 = 1:orders
      for nO2 = nO1:orders
         E_Pm3(end+1) = mean(uu(:,nO2-nO1+1));
      end
   end
end
...
for nO1 = 1:orders
   ...
   for nO2 = 1:nO1
      E_PmPr3(nO1+1,nO2+1) = mean(uu(:,nO1-nO2+1));
   end
end
...
if (nlinear == 2)
   ...
   for nO1b = 1:orders
      for nO2 = 1:orders
         uuu(nO1b,nO2) = mean(uu(:,nO1b).*u(nO2:end-orders+nO2));
      end
   end
   ...
   for nO1b = 1:orders
      for nO2b = 1:orders
         uuuu(nO1b,nO2b) = mean(uu(:,nO1b).*uu(:,nO2b));
      end
   end
   ...
   for nO1a = 1:orders
    for nO1b = nO1a:orders
      ...
      E_PmPr3(nO1+1,1) = mean(uu(:,nO1b-nO1a+1));

      for nO2 = 1:orders
         E_PmPr3(nO1+1,nO2+1) = uuu(nO1b-nO1a+1,nO2);
      end
      ...
      for nO2a = 1:orders
       for nO2b = nO2a:orders
        if nO2 < nO1
         nO2 = nO2 + 1;
         E_PmPr3(nO1+1,nO2+1) = uuuu(nO1b-nO1a+1,nO2b-nO2a+1);
	end
       end
      end
    end
...
for m = 2:lp_m
   for r = 2:m-1
      i = [1:r-1];
      D(m,r) = E_PmPr3(m,r) - alpha(r,i)*D(m,i)';
   end
